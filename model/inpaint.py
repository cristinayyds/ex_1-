...
def context_loss(corrupted_images: object, generated_images: object, masks: object, weighted: object = True) -> object:
...
def posisson_blending(masks,generated_images,corrupted_images):
    ...
    image_optimum = nn.Parameter(torch.FloatTensor(initial_guess.detach().cpu().numpy()))
...
def inpaint(args):
    ...
    for i, (corrupted_images, original_images, masks, weighted_masks) in enumerate(dataloader):
        ...
        z_optimum = nn.Parameter(torch.FloatTensor(np.random.normal(0, 1, (corrupted_images.shape[0],args.latent_dim,))).to(device))
        ...
        save_image(corrupted_images, "../output3(win_size3_255)/corrupted_{}.png".format(i), normalize=True, range=image_range, nrow=5)
        save_image(generated_images, "../output3(win_size3_255)/output_{}.png".format(i), normalize=True, range=image_range, nrow=5)
        save_image(blended_images, "../output3(win_size3_255)/blended_{}.png".format(i), normalize=True, range=image_range, nrow=5)
        save_image(original_images, "../output3(win_size3_255)/original_{}.png".format(i), normalize=True, range=image_range, nrow=5)
